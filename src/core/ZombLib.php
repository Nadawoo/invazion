...
class ZombLib
{
     ...
     *                          e.g. : ['map_id' => 1]
     * @param  string $method   The HTTP method to use to send the data to the API
     *                          (GET or POST)
     * @param bool $add_token If set to TRUE, the user's token will be automatically 
     *                        added to the request to the API
     * @return array  The result generated by the API (JSON converted to an array)
     * @throws Exception
     */
    public function call_api($api_name, $action, $params=[], $method='GET', $add_token=true)
    {   
        ...
        // Add token only if requested
        if ($add_token === true) {
            $base_params['token'] = $this->get_token();
        }        
        // Builds the parameters to send to the API, encoded like url parameters (e.g.: "action=get&map_id=1")        
        $api_params = http_build_query($base_params + (array)$params);
